...
class PDFGenerator:
    ...
    def create_custom_styles(self):
        ...
        custom_styles.add(ParagraphStyle(name="FileContent", fontSize=10, leading=12, spaceAfter=6, fontName='Helvetica'))
        custom_styles.add(ParagraphStyle(name="Emoji", fontSize=10, leading=12, fontName='NotoEmoji'))
        return custom_styles

    def create_pdf(self, root_node: FileNode, root_path: str):
        self.doc = SimpleDocTemplate(self.output_path, pagesize=letter)
        self.story.append(Paragraph("Directory and File Contents", self.styles["Title"]))
    ...
    def format_file_tree(self, node: FileNode, indent: str):
        if node.node_type == 'directory':
            self.story.append(Paragraph(f"{indent}<font name='NotoEmoji'>📁</font> {node.name}", self.custom_styles["FileTree"]))
            for child in node.children:
                self.format_file_tree(child, indent + "&nbsp;&nbsp;&nbsp;&nbsp;")
        else:
            section_anchor = f"section_{len(self.toc)}"
            self.toc.append((node.path, section_anchor))
            self.story.append(Paragraph(f"{indent}<font name='NotoEmoji'>📄</font> <a href=\"#{section_anchor}\">{node.name}</a>", self.custom_styles["FileTree"]))
        ...
        if node.node_type == 'file':
            ...
            for i, (path, anchor) in enumerate(self.toc):
                if path == node.path:
                    ...
                    break
            if section_anchor:
                self.story.append(Paragraph(f'<a name="{section_anchor}"></a>{node.path}', self.styles["Heading2"]))
                self.story.append(Spacer(1, 0.1 * inch))
                self.story.append(Paragraph(node.content.replace("\n", "<br />"), self.custom_styles["FileContent"]))
                self.story.append(PageBreak())
    ...
    def add_table_of_contents(self):
        ...
        for file_path, section_anchor in self.toc:
            ...
            toc_entries.append([Paragraph(f'<a href="#{section_anchor}">{relative_path}</a>', self.custom_styles["TOC"])])
        if toc_entries:
            table = Table(toc_entries, colWidths=[7 * inch])
            table.setStyle(TableStyle([("TEXTCOLOR", (0, 0), (-1, -1), colors.blue), ("VALIGN", (0, 0), (-1, -1), "TOP")]))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate PDF of directory tree and file contents.")
